...
export default function Dashboard() {
  ...
  const activeBookingsCount = Object.keys(state.bookings).length;

  const stats = useMemo(() => [
    { title: "Total Slot", value: totalSlots, trend: 0 },
    { title: "Slot Terisi", value: occupiedSlots, trend: 0, color: "danger" as const },
    {
      title: "Slot Kosong",
  ...
  const gridSlots: ParkingSlotData[] = useMemo(() => filteredSlots.map((s) => ({
    id: s.id,
    status: s.status === "occupied" ? "occupied" : "available",
    disabled: s.disabled || s.status === "maintenance",
  })), [filteredSlots]);

  const mapSlots: ParkingSlotMapData[] = useMemo(() => filteredSlots.map((s) => ({
    id: s.id,
    status: s.status === "occupied" ? "occupied" : "available",
    x: s.x,
    y: s.y,
  ...
  const handleSlotSelect = useCallback((slotId: string) => {
    ...
    } else if (selectedSlot && selectedSlot.status === "available" && !selectedSlot.disabled) {
  ...
  const modalReceiptRows = useMemo(() => {
    ...
    return [
      ...
      { label: "DURASI PESANAN", value: `${occupiedModalData.booking.durationHours} Jam`, borderBottom: false },
    ];
  }, [occupiedModalData]);
  ...
  return (
    <div className="flex flex-col gap-8 w-full text-slate-800 dark:text-slate-100">
      ...
      <Card className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4">
        <div>
          <h4 className="font-bold text-slate-900 dark:text-white text-sm">Filter Ukuran Kendaraan</h4>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Saring tampilan kavling parkir berdasarkan dimensi mobil anda</p>
        </div>
        <select
          value={state.filters?.vehicleSize || ""}
          onChange={(e) => {
            const val = e.target.value;
            updateFilter({ vehicleSize: val ? (val as "small" | "medium" | "large") : undefined });
          }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start w-full">
        <section className="lg:col-span-6 flex flex-col gap-3">
          <h3 className="text-lg font-bold text-slate-800 dark:text-white">Tata Letak Peta Zona</h3>
          <ParkingMap
            slots={mapSlots}
        ...
        <section className="lg:col-span-6 flex flex-col gap-3">
          <h3 className="text-lg font-bold text-slate-800 dark:text-white">Daftar Kisi Slot Langsung</h3>
          <ParkingGrid
            slots={gridSlots}
            selectedSlotId={state.selectedSlotId || undefined}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in">
          <Card className="max-w-md shadow-2xl flex flex-col gap-5 text-left bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
            <header className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
              <div>
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Slot Terisi</h3>
            ...
            {occupiedModalData.booking ? (
              ...
              <div className="flex flex-col gap-4">
                <p className="text-sm text-slate-600 dark:text-slate-400">
                  Slot parkir ini saat ini ditandai sebagai terisi, tetapi tidak ada sesi pembayaran aktif yang tercatat dalam log sistem.
                </p>
                <Button
                  variant="secondary"
                  onClick={() => setOccupiedModalData(null)}
